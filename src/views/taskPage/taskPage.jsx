...
class TaskPage extends React.Component {
  ...
  sortOutTasks(tasks) {
    ...
    for (let task of tasks) { //accumulate every single task 
      if (!taskList[task.category]) { //if there hasn't been that said category before, create the array 
  ...
  render() {
    ...
    if (this.props.tasks) {
      ...
      listOfTasks = taskObj.categories.map((category, index) => ( //for every category, create a list of Tasks
        <ListOfTasks key={index} items={taskObj.taskList[category]} category={category}/>
      ));
    }
